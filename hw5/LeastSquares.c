...
/* Draw Least Squares Polynomial
 * Create the Window to draw the Least Squares Polynomial. Wait for user
 * to click on the window and store the clicks as data points,
 * Lastly, draw the Least Squares polynomial.
 */
int draw_least_squares(int size)
{
    int swidth;
    int sheight;
    ...
    G_save_image_to_file("LeastSqauares_SteveBraich.xwd");
}
...
int main()
{
    ...
    {
        printf("\n\n");
        printf("\tLeast Squares Polynomial Regression Graphing Tool\n");
        printf("\t----------------------------------------------------\n\n");
        ...
        char *ptr;
        int size = strtol(menu, &ptr, 10);

        if (size > 0 && size < 101)
        {
            draw_least_squares(size);
        }
